    ...
    try {
      ...
      */
      ...
      if (
        ...
        params.set("salaryMin", String(searchData.salaryMin));
      }
      ...
      if (
        ...
        params.set("salaryMax", String(searchData.salaryMax));
      }

      if (searchData.salaryCurrency) {
        params.set("salaryCurrency", searchData.salaryCurrency);
      }

      const queryString = params.toString();

      console.log("SEARCHING DATABASE:", queryString);
      ...
      */
      ...
      const jobsResponse = await fetch(`/api/jobs?${queryString}`);

      const jobsData = await jobsResponse.json();

      if (!jobsResponse.ok) {
        throw new Error(jobsData.message || "Could not search jobs.");
      }

      console.log("DATABASE MATCHES:", jobsData);
      ...
      */
      ...
      /*
        -----------------------------------------
        STEP 6
        Ask Groq to write a natural reply based on
        the filters used and how many jobs matched
        -----------------------------------------
      */

      const replyResponse = await fetch("/api/chatboat", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
