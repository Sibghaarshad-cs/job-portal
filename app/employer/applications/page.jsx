...
export default function ApplicationsPage() {
  ...
  useEffect(() => {
    async function loadApplications() {
      setLoading(true);
      setError(null);
      const target = jobId ? `/api/employer/applications?jobId=${jobId}` : "/api/employer/applications";
      const response = await fetch(target);

      if (response.status === 401) {
        router.push(`/login?next=${encodeURIComponent(`/employer/applications${jobId ? `?jobId=${jobId}` : ""}`)}`);
        return;
      }
        ...
        <div className="p-8">
          <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex items-center gap-3">
              <button
                type="button"
                onClick={() => router.push(jobId ? "/employer/manage-jobs" : "/employer/dashboard")}
          ...
          {loading ? (
            <div className="rounded-3xl bg-white border border-gray-200 p-6 shadow-sm text-center">
              Loading applications...
            </div>
          ) : error ? (
            <div className="rounded-3xl bg-white border border-red-200 p-6 shadow-sm text-center text-red-600">
              {error}
            </div>
          ) : (
            <div className="rounded-3xl bg-white border border-gray-200 p-6 shadow-sm">
              <div className="mb-6">
                <h2 className="text-xl font-semibold text-gray-900">{applicationCount} Applications</h2>
              </div>
              <div className="space-y-4">
                {applications.length === 0 ? (
                  <div className="rounded-3xl border border-gray-200 bg-white p-10 shadow-sm text-center">
                    <p className="text-xl font-semibold text-gray-900">No applications yet</p>
                    <p className="text-gray-500 mt-2">Once someone applies to your job, they'll appear here.</p>
                  </div>
                ) : (
                  applications.map((application) => (
                    <div
                      key={application.id}
                      className="flex flex-col gap-4 rounded-3xl border border-gray-200 bg-white p-6 shadow-sm transition hover:shadow-md sm:flex-row sm:items-center sm:justify-between"
                    >
                      <div className="flex flex-1 items-start gap-4">
                        ...
                        <div>
                          <h3 className="text-base font-semibold text-gray-900">{application.user.name}</h3>
                          ...
                          <p className="mt-2 flex items-center gap-2 text-sm text-gray-500">
                            <span className="inline-flex h-6 w-6 items-center justify-center rounded-full bg-slate-100 text-slate-500">
                              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="h-4 w-4">
                                ...
                                <path fillRule="evenodd" d="M2.25 12a9.75 9.75 0 1119.5 0 9.75 9.75 0 01-19.5 0zm9.75-7.5a7.5 7.5 0 00-6.28 12.14A6.244 6.244 0 0112 15.75a6.244 6.244 0 015.03 1.89A7.5 7.5 0 0012 4.5z" clipRule="evenodd" />
                              </svg>
                            </span>
